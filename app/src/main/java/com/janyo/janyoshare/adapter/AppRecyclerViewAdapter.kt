...
class AppRecyclerViewAdapter(private val context: Context,
    ...
    private val spotsDialog = ZLoadingDialog(context)
            .setLoadingBuilder(Z_TYPE.CIRCLE_CLOCK)
            .setHintText(context.getString(R.string.hint_upload_log))
            .setHintTextSize(16F)
    ...
    private val option = RequestOptions()
            ...
            .signature(MediaStoreSignature("text/plain",settings.imgVersion.toLong(),0))
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        spotsDialog.create().setOnCancelListener(null)
        spotsDialog.setHintText(context.getString(R.string.copy_file_loading))
        ...
        if (settings.isDisableIcon) {
            Glide.with(context).load(R.mipmap.ic_launcher).apply(option).into(holder.imageView)
        } else {
            if (installApp.iconPath != null)
                Glide.with(context).load(installApp.iconPath).apply(option).into(holder.imageView)
            else
